...
Z_FORCEINLINE static void adler32_copy_small(uint32_t *Z_RESTRICT adler, uint8_t *dst, const uint8_t *buf,
    size_t len, uint32_t *Z_RESTRICT sum2, const int MAX_LEN, const int COPY) {
    if (COPY) {
        memcpy(dst, buf, len);
    }
    if (MAX_LEN > 16) {
